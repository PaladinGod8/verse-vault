...
| `src/renderer/pages/BattleMapRuntimePage.tsx`                  | BattleMap runtime page (`/world/:id/battlemaps/:battleMapId/runtime`): validates world/battlemap route params, loads and normalizes runtime config JSON, renders runtime grid controls (mode/cell size/origin), debounces + persists `runtime.grid` updates via `window.db.battlemaps.update` with safe config merging, mounts `BattleMapRuntimeCanvas`, and keeps a mandatory `Exit Runtime` button visible to return to battlemaps list context                                                                                                                                                                                                                          |
