...
| `src/renderer/pages/SessionsPage.tsx`                          | Sessions list page (`/world/:id/campaign/:campaignId/sessions`): validates worldId + campaignId, loads campaign header + sessions, supports create/edit/delete, Scenes link per row (`/world/:id/campaign/:campaignId/session/:sessionId/scenes`), and loading/empty/error states                                                      |
...
| `forge.env.d.ts`                                               | Global TS types: `Verse`, `World`, `Level`, `Ability`, `AbilityChild`, `Campaign`, `Session`, `Scene`, `DbApi`, Vite constants                                                                                                                                                                                                         |
...
- **Purpose**: add session CRUD handlers in main process scoped by campaign with explicit partial-update behavior
- **Status**: added on 2026-02-27
- **UI**: none in this step
- **Store**: none yet
- **IPC**: `IPC.SESSIONS_GET_ALL_BY_CAMPAIGN`, `IPC.SESSIONS_GET_BY_ID`, `IPC.SESSIONS_ADD`, `IPC.SESSIONS_UPDATE`, `IPC.SESSIONS_DELETE`
