...
| `src/renderer/pages/TokensPage.tsx`                            | World-level token list page (`/world/:id/tokens`): table of world + campaign-scoped tokens with scope label, thumbnail, create/edit/delete actions via shared `ModalShell` + `ConfirmDialog`, create flow image-upload handoff (`window.db.tokens.importImage`) before `tokens.add`, "Copy to Campaign" action for world-scoped rows via `CopyTokenToCampaignDialog`, toast-first mutation feedback, and loading/empty/error states                                                                                                                                                                                                                                        |
...
| `src/renderer/components/tokens/TokenForm.tsx`                 | Reusable token form for create/edit (name required, optional image URL, visible checkbox) plus create-only token image upload payload wiring (`image_upload`) from dropzone/file picker                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
