...
| Path                                                                   | Responsibility                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/main.ts`                                                          | App bootstrap, BrowserWindow creation, IPC handler registration (`verses` CRUD + `worlds` read/create/update/delete/markViewed/import-image + `levels` read + `levels` create/update/delete + `campaigns` read/create/update/delete + `battlemaps` read/create/update/delete + `tokens` read-by-world/read-by-campaign/get-by-id/import-image/create/update/delete/moveToWorld/moveToCampaign + `arcs` read/create/update/delete + `acts` read/create/update/delete/moveTo + `sessions` read/create/update/delete/moveTo + `scenes` read-by-campaign/read-by-session/create/update/delete/moveTo + `abilities` read + `abilities` add/update/delete/addChild/removeChild + `statblocks` read/create/update/delete/linkToken/unlinkToken/getLinkedTokens/getLinkedStatblock/attachAbility/detachAbility/listAbilities); registers `vv-media://` protocol serving both `token-images` and `world-images` hosts for secure local image serving |
| `src/preload.ts`                                                       | contextBridge - exposes `window.db` (`verses` CRUD + `worlds` read/create/update/delete/markViewed/import-image + `levels` read/add/update/delete + `abilities` read/add/update/delete/addChild/removeChild + `campaigns` read/add/update/delete + `battlemaps` read/add/update/delete + `tokens` read-by-world/read-by-campaign/get-by-id/import-image/add/update/delete/moveToWorld/moveToCampaign + `arcs` read/add/update/delete + `acts` read/add/update/delete/moveTo + `sessions` read/add/update/delete/moveTo + `scenes` read-by-campaign/read-by-session/add/update/delete/moveTo + `statblocks` read/add/update/delete/linkToken/unlinkToken/getLinkedTokens/getLinkedStatblock/attachAbility/detachAbility/listAbilities) to renderer                                                                                                                                                                                           |
| `src/database/db.ts`                                                   | SQLite singleton, schema init (`verses`, `worlds`, `levels`, `campaigns`, `battlemaps`, `tokens`, `arcs`, `acts`, `sessions`, `scenes`, `abilities`, `ability_children`, `characters`, `statblocks`, `statblock_token_links`, `statblock_ability_assignments`), additive migrations (`runArcActMigration`, `runSessionPlannedAtMigration`, `runTokenWorldIdMigration`, `runTokenCampaignNullableMigration`, `runTokenGridTypeMigration`, `runAbilitiesRangeShapeTargetMigration`, `runStatBlocksSchemaMigration`, `runStatBlockLinkageSchemaMigration`, `runWorldConfigMigration`), token move transactional helpers (`db.tokens.moveToWorld`, `db.tokens.moveToCampaign`), shared token config JSON validation/normalization helper (`ensureTokenConfigJsonText`) for footprint/framing metadata, open/close                                                                                                                               |
| `src/shared/ipcChannels.ts`                                            | All IPC channel name constants (single source of truth) for verses, worlds (including world image import), levels, abilities, campaigns, battlemaps, tokens (including token image import and token move channels), arcs, acts, sessions, scenes, and statblocks (CRUD + token linkage + ability assignment) contracts                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `src/shared/statisticsTypes.ts`                                        | Statistics type definitions: `WorldStatisticsConfig` (world-level statistics configuration), `ResourceStatisticDefinition` (resource definition shape with id/name/abbreviation/description/isDefault), `PassiveScoreDefinition` (passive score definition shape with id/name/abbreviation/description/type/isDefault), `PassiveScoreType` (ability_score/proficiency_bonus/custom), `StatBlockStatisticsConfig` (statblock-level statistics config), `StatBlockResourceValue` (current/maximum resource values), `StatBlockPassiveScoreValue` (base values + calculated modifiers for passive scores); exports `DEFAULT_RESOURCE_DEFINITIONS` (HP/MP/AC) and `DEFAULT_PASSIVE_SCORE_DEFINITIONS` (STR/DEX/CON/INT/WIS/CHA/PB) for D&D 5e                                                                                                                                                                                                   |
...
| `src/renderer/pages/WorldStatisticsConfigPage.tsx`                     | World statistics configuration page (`/world/:id/statistics`): validates world route param, loads world config JSON, parses world-level statistics (resources + passive scores), renders resource CRUD and passive score CRUD tables with create/edit/delete actions via `ResourceDefinitionForm` + `PassiveScoreDefinitionForm` + `ConfirmDialog`, persists config updates via `window.db.worlds.update`, provides toast-first mutation feedback, and includes loading/error states                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `src/renderer/pages/BattleMapRuntimePage.tsx`                          | BattleMap runtime page (`/world/:id/battlemaps/:battleMapId/runtime`): validates world/battlemap route params, loads normalized runtime config JSON, debounces + persists runtime grid/camera/map config updates via `window.db.battlemaps.update`, loads world-scoped tokens plus selected-campaign tokens for placement, filters tokens by grid_type matching active grid mode (square/hex), tracks runtime token instances (select/remove/position), opens `StatBlockPopup` on token double left-click, blocks mismatched grid-type token placement when grid mode is not 'none', blocks in-app navigation when unsaved runtime config changes are pending until save/confirm resolves, mounts `BattleMapRuntimeCanvas`, and keeps a mandatory `Exit Runtime` button visible to return to battlemaps list context                                                                                                                        |
...
| `src/renderer/components/statistics/ResourceDefinitionForm.tsx`        | Reusable resource definition form for create/edit (ID immutable in edit mode, name + abbreviation required, optional description, default-inclusion checkbox); validates ID uniqueness + lowercase format; handles pending state and error feedback                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `src/renderer/components/statistics/PassiveScoreDefinitionForm.tsx`    | Reusable passive score definition form for create/edit (ID immutable in edit mode, name + abbreviation + type required, optional description, default-inclusion checkbox); type dropdown supports ability_score/proficiency_bonus/custom; validates ID uniqueness + lowercase format; handles pending state and error feedback                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `src/renderer/components/statistics/ResourceStatisticInput.tsx`        | Reusable resource statistic input component for statblock forms (current/maximum value fields); validates current ≤ maximum with auto-clamping on maximum change; displays resource name/abbreviation/description from world definition; handles disabled state and error feedback                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `src/renderer/components/statistics/PassiveScoreInput.tsx`             | Reusable passive score input component for statblock forms (base value field); displays passive score name/abbreviation/description from world definition; calculates and displays ability modifier (read-only) for ability_score type; calculates and displays save modifier (read-only) for ability scores; shows modifier formula helper text; handles disabled state                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `src/renderer/components/runtime/AbilityPickerPanel.tsx`               | Floating ability picker panel: loads active abilities from the selected runtime token's linked statblock (`window.db.statblocks.getLinkedStatblock` + `listAbilities`), lists ability range/AoE metadata, highlights the selected casting ability, and calls `onAbilitySelect` to enter/exit cast mode; shown above the runtime canvas when a token is selected                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `src/renderer/components/runtime/BattleMapRuntimeCanvas.tsx`           | PixiJS runtime canvas: mounts/unmounts `Application`, initializes runtime stage graph containers (`world/background/map/image/grid/rangeOverlay/token/ui`), renders square/hex/none grid overlays from runtime config, renders token sprites/fallback token markers in token layer, supports token select + pointer drag with square/hex/none snap-on-drop, supports drag-to-pan on vacant background, supports token double left-click callback emission from pointer-up timing logic, smoothly centers camera on selected tokens, cancels focus animation when manual pan begins, applies camera transform, renders a black fallback background whenever map image is absent, loads optional runtime map image without recreating stage objects on React re-render, and renders a casting range overlay (range ring, AoE shape fill, tile highlights) when `castingState` is set; tracks pointer angle for cone/line shapes               |
| `src/renderer/components/runtime/StatBlockPopup.tsx`                   | Runtime statblock modal popup (same BrowserWindow): opens from runtime token double-click, resolves linked statblock + assigned abilities for the token, renders statblock details/resources/passive scores/skills, and allows cast-selection toggles for range-capable abilities without leaving runtime                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `src/renderer/lib/statisticsCalculations.ts`                           | Pure D&D 5e statistics calculation library: `calculateAbilityModifier` and `calculateSaveModifier` compute modifiers from scores using floor((score - 10) / 2), `formatModifier` adds +/- sign formatting for display, `isResourceValueValid` validates resource bounds (current ≤ maximum, both ≥ 0), `clampResourceValue` clamps resource values to [0, maximum], `isPassiveScoreValid` validates passive score base values (0-99), `validateResourceStatistic` validates resource definition fields (ID, name, abbreviation with max 5 chars), `validatePassiveScoreDefinition` validates passive score definition fields (ID, name, abbreviation with max 5 chars, type); all functions handle non-finite inputs gracefully                                                                                                                                                                                                             |
| `src/renderer/lib/statblockStatisticsUtils.ts`                         | Statblock statistics read/write utilities: `parseStatBlockStatistics` parses JSON with graceful error fallback, `getResourceValue`/`getPassiveScoreValue` type-safe getters, `setResourceValue`/`setPassiveScoreValue` immutable setters, `initializeStatBlockStatistics` initializes from world defaults (isDefault: true only), `serializeStatBlockStatistics` JSON serialization; all helpers enforce immutability in setter chains                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `forge.env.d.ts`                                                       | Global TS types: `Verse`, `World` (including `config: string`), `Level`, `Ability`, `AbilityChild`, `Campaign`, `BattleMap`, token contracts (`Token`, `TokenGridType`, `TokenSquareFootprintCell`, `TokenHexFootprintCell`, `TokenFootprintConfig`, `TokenFramingConfig`, `TokenConfigShape`, `TokenImageImportPayload`, `TokenImageImportResult`), `Arc`, `Act`, `Session`, `Scene`, `CampaignSceneListItem`, `DbApi` (including token move signatures and worlds `config?: string` add/update payloads), Vite constants                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `dprint.json`                                                          | dprint formatter configuration (Rust-based formatter for CI/dev workflows): TypeScript/JSON/Markdown plugins, single-quote style, semicolons always, 100-char line width, excludes node_modules/build artifacts; provides significantly faster format checking than prettier for batch operations                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `playwright.config.ts`                                                 | Playwright E2E test configuration: uses project-based worker groups (`smoke`/`medium`/`runtime`) for mixed test-weight scheduling; smoke tests (app, statblocks-crud, statblocks) run with higher parallelization; medium tests (abilities, battlemaps, world-statistics-config, statblock-statistics, casting-range-overlay, tokenMove) run with moderate parallelization; runtime tests (arc-act, battlemap-runtime-play, tokens) run with conservative workers; run all groups via `yarn test:e2e` or target one group with `--project=<name>`; default worker cap is 2 with optional local override via `PLAYWRIGHT_WORKERS` (for example `yarn test:e2e:local:8`)                                                                                                                                                                                                                                                                     |
...
| `tests/unit/statisticsCalculations.test.ts`                            | Unit coverage for statistics calculation library: ability/save modifiers, modifier formatting, resource validation (isResourceValueValid, clampResourceValue), passive score validation (isPassiveScoreValid), resource definition validation (validateResourceStatistic with ID/name/abbreviation checks), passive score definition validation (validatePassiveScoreDefinition with ID/name/abbreviation/type checks); includes edge cases for non-finite values (NaN, Infinity, negative) and validation boundary conditions                                                                                                                                                                                                                                                                                                                                                                                                              |
| `tests/unit/renderer/lib/statisticsCalculations.test.ts`               | (Legacy location - tests moved to tests/unit/statisticsCalculations.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `tests/unit/renderer/lib/statblockStatisticsUtils.test.ts`             | Unit coverage for statblock statistics utilities: JSON parsing with error fallback, type-safe getters/setters, immutability enforcement, world definition initialization (default values only), and serialization contracts; includes edge cases for invalid JSON and nested object preservation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
### World Config Schema (Step 01)

- **Purpose**: extend worlds storage with a world-level JSON config field for system/scaffold settings while staying migration-safe on existing user databases
- **Status**: added on 2026-03-06
- **UI**: none in this step
- **Store**: none yet
- **IPC**: unchanged in this step
- **Main handler**: unchanged in this step
- **Storage**: `verse-vault.db` -> `worlds` table now includes `config` (`TEXT NOT NULL DEFAULT '{}'`); additive migration `runWorldConfigMigration()` checks `PRAGMA table_info(worlds)` and runs `ALTER TABLE worlds ADD COLUMN config ...` only when missing

### World Config IPC (Step 02)

- **Purpose**: wire worlds add/update IPC to accept and persist world-level JSON config text.
- **Status**: added on 2026-03-06.
- **UI**: none in this step.
- **Store**: none yet.
- **IPC**: extends existing `IPC.WORLDS_ADD` and `IPC.WORLDS_UPDATE` payloads with optional `config?: string`.
- **Main handler**: `src/main.ts` -> `registerIpcHandlers()` validates `config` JSON text and persists it.
- **Preload bridge**: unchanged method surface (`window.db.worlds.add/update`), now typed for optional `config` in shared contract.
- **Storage**: `WORLDS_ADD` inserts `config` (default `'{}'` when omitted); `WORLDS_UPDATE` writes `config` only when provided via `hasOwnProperty`, with JSON validation before persistence.
...
- **IPC**: `IPC.WORLDS_ADD`
- **Main handler**: `src/main.ts` -> `registerIpcHandlers()`
- **Storage**: inserts into `worlds` (`name`, `thumbnail`, `short_description`, `config`), validates `name.trim()` is non-empty, validates optional `config` as JSON text (defaults to `'{}'`), then returns `SELECT * FROM worlds WHERE id = ?`
...
- **Main handler**: `src/main.ts` -> `registerIpcHandlers()`
- **Storage**: `WORLDS_UPDATE` mutates only provided fields (`name`, `thumbnail`, `short_description`, `config`) and sets `updated_at = datetime('now')`; `config` is JSON-validated and defaults to `'{}'` when property is present but not a string. `WORLDS_DELETE` removes by id and returns `{ id }`; `WORLDS_MARK_VIEWED` sets `last_viewed_at = datetime('now')` and returns the updated row or `null` if missing
...
### StatBlock Linkage Backbone (Step 01)

- **Purpose**: add minimal runtime backbone for token-to-statblock linkage, statblock-to-ability assignment, and MVP statblock skills config normalization
- **Status**: added on 2026-03-07
- **UI**: none in this step
- **Store**: none
- **IPC**: adds `IPC.STATBLOCKS_LINK_TOKEN`, `IPC.STATBLOCKS_UNLINK_TOKEN`, `IPC.STATBLOCKS_GET_LINKED_TOKENS`, `IPC.STATBLOCKS_GET_LINKED_STATBLOCK`, `IPC.STATBLOCKS_ATTACH_ABILITY`, `IPC.STATBLOCKS_DETACH_ABILITY`, `IPC.STATBLOCKS_LIST_ABILITIES`
- **Main handler**: `src/main.ts` -> `registerIpcHandlers()` adds all 7 handlers, enforces same-world checks on link/attach (`token.world_id`/`ability.world_id` must match `statblock.world_id`), and returns explicit duplicate-link/duplicate-assignment errors
- **Preload bridge**: `src/preload.ts` adds `window.db.statblocks.linkToken/unlinkToken/getLinkedTokens/getLinkedStatblock/attachAbility/detachAbility/listAbilities`
- **Storage**: `src/database/db.ts` adds `runStatBlockLinkageSchemaMigration(db)` creating `statblock_token_links` (one token linked to at most one statblock via `UNIQUE(token_id)`) and `statblock_ability_assignments` (many-to-many with unique `(statblock_id, ability_id)`), plus supporting indexes
- **Types**: `forge.env.d.ts` adds `StatBlockTokenLink`, `StatBlockAbilityAssignment`, `StatBlockSkillValue`, `StatBlockConfig`, and extends `DbApi.statblocks` with the 7 new methods; `StatBlock.config` documents optional MVP `skills` support
- **Validation**: `STATBLOCKS_ADD`/`STATBLOCKS_UPDATE` now validate `config` JSON object text and normalize optional `skills` as deterministic `[{ key, rank }]`
- **Tests**: none in this code step (Phase 2 owns test updates)
...
### StatBlock Resource UI (Step 10)

- **Purpose**: extend statblock form to display and edit resource statistics based on world configuration; show current/maximum inputs for each resource defined in the world
- **Status**: added on 2026-03-06
- **UI**:
  - `src/renderer/components/statistics/ResourceStatisticInput.tsx` — new input component with current/maximum fields; validates current ≤ maximum with live error messages; auto-clamps current value when maximum changes; displays resource name/abbreviation/description from world definition
  - `src/renderer/components/statblocks/StatBlockForm.tsx` — extended: loads world via `window.db.worlds.getById`, parses world statistics config JSON, displays grid of `ResourceStatisticInput` components for each world resource definition, initializes empty statistics for create mode via `initializeStatBlockStatistics`, parses existing statistics from statblock config for edit mode via `parseStatBlockStatistics`, serializes statistics config on submit via `serializeStatBlockStatistics`, merges serialized statistics into statblock config when resources are present
- **Store**: none
- **IPC**: uses existing `window.db.worlds.getById` (from World feature) and `window.db.statblocks.add/update` (from Steps 03-05)
- **Main handler**: unchanged
- **Preload bridge**: unchanged
- **Storage**: no schema change; uses existing `statblocks.config` JSON column
- **Types**: uses `WorldStatisticsConfig`, `ResourceStatisticDefinition`, `StatBlockStatisticsConfig`, `StatBlockResourceValue` from `src/shared/statisticsTypes.ts` (Step 07); no new types added
- **Tests**: none added in this step; relies on existing unit tests for `statblockStatisticsUtils` and manual verification

### StatBlock Passive Scores UI (Step 11)

- **Purpose**: extend statblock form to display and edit passive scores (ability scores, proficiency bonus) with calculated modifiers; show base value inputs and read-only calculated modifiers
- **Status**: added on 2026-03-06
- **UI**:
  - `src/renderer/components/statistics/PassiveScoreInput.tsx` — new input component with base value field; displays passive score name/abbreviation/description from world definition; calculates and displays ability modifier (read-only) using floor((baseValue - 10) / 2) for ability_score type; calculates and displays save modifier (read-only) for ability scores; shows modifier formula helper text
  - `src/renderer/components/statblocks/StatBlockForm.tsx` — extended: loads world passive score definitions alongside resources, displays grid of `PassiveScoreInput` components for each world passive score definition, initializes default passive scores (baseValue: 10) in create mode via `initializeStatBlockStatistics(worldResources, worldPassiveScores)`, parses existing passive scores from statblock config for edit mode, serializes passive scores config on submit when present
- **Store**: none
- **IPC**: uses existing `window.db.worlds.getById` (from World feature) and `window.db.statblocks.add/update` (from Steps 03-05)
- **Main handler**: unchanged
- **Preload bridge**: unchanged
...
### StatBlock Validation and Edge Cases (Step 13)

- **Purpose**: add comprehensive validation helpers for statistics definitions, handle edge cases when world statistics are modified or deleted, ensure graceful behavior without crashes
- **Status**: added on 2026-03-06
- **UI**:
  - `src/renderer/lib/statisticsCalculations.ts` — extended: adds `isPassiveScoreValid(baseValue)` to validate passive scores in range 0-99, `validateResourceStatistic(definition)` to validate all required resource fields (ID, name, abbreviation with max 5 chars), `validatePassiveScoreDefinition(definition)` to validate all required passive score fields (ID, name, abbreviation with max 5 chars, type)
  - `src/renderer/components/statblocks/StatBlockForm.tsx` — extended: imports `useMemo` from React; adds `filteredStatistics` memo hook that filters statistics config to only include resources and passive scores still defined in world config (excludes deleted definitions); uses filtered statistics in submit handler to prevent saving orphaned statistics
  - `src/renderer/components/statblocks/StatBlockCard.tsx` — extended: adds defensive type checks for resource values (validates `current` and `maximum` are numbers); guards against missing resource values; provides fallback abbreviation ('STAT') when abbreviation is missing or empty
- **Store**: none
- **IPC**: uses existing `window.db.worlds.getById` (from World feature) and `window.db.statblocks.add/update` (from Steps 03-05)
- **Main handler**: unchanged
- **Preload bridge**: unchanged
- **Storage**: no schema change; filtered statistics prevent orphaned statistics from being saved in `statblocks.config` JSON
- **Types**: no new types added; uses existing types from `src/shared/statisticsTypes.ts`
- **Tests**: `tests/unit/statisticsCalculations.test.ts` — comprehensive unit coverage for all validation functions including isResourceValueValid, clampResourceValue, isPassiveScoreValid, validateResourceStatistic, validatePassiveScoreDefinition; includes edge cases for boundary validation, non-finite values, and field presence checks; 29 total tests all passing
...
### Runtime StatBlock Popup and Ability Source Refactor (Step 05)

- **Purpose**: shift runtime ability selection from world-level lists to selected-token linked statblock abilities, and add in-app runtime statblock popup on token double left-click
- **Status**: added on 2026-03-07
- **UI**:
  - `src/renderer/components/runtime/AbilityPickerPanel.tsx` - refactored to resolve selected token linked statblock and load active abilities from `window.db.statblocks.listAbilities`
  - `src/renderer/components/runtime/BattleMapRuntimeCanvas.tsx` - adds `onTokenDoubleClick` callback and double-click detection via no-move pointer-up timing
  - `src/renderer/components/runtime/StatBlockPopup.tsx` - new in-app modal popup for linked statblock details, abilities, resources, passive scores, and skills
  - `src/renderer/pages/BattleMapRuntimePage.tsx` - wires popup open/close state, canvas double-click callback, and statblock-driven ability picker props
- **Store**: none
- **IPC**: unchanged (uses existing `window.db.statblocks.getLinkedStatblock` and `window.db.statblocks.listAbilities`)
- **Main handler**: unchanged
- **Preload bridge**: unchanged
- **Storage**: no schema change
- **Types**: no new global types
- **Tests**: none in this code step (Phase 2 will add/update tests)
