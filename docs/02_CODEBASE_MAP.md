...
| `src/renderer/pages/BattleMapRuntimePage.tsx`                  | BattleMap runtime page (`/world/:id/battlemaps/:battleMapId/runtime`): validates world/battlemap route params, loads battlemap + validates config JSON object shape, normalizes runtime config defaults, mounts `BattleMapRuntimeCanvas`, and keeps a mandatory `Exit Runtime` button visible to return to battlemaps list context                                                                                                                                                                                                                                                                                                                                         |
...
| `src/renderer/components/runtime/BattleMapRuntimeCanvas.tsx`   | PixiJS runtime canvas bootstrap: mounts/unmounts `Application`, initializes runtime stage graph containers (`world/background/map/image/grid/token/ui`), renders base background + map layers, applies camera transform, and loads optional runtime map image without recreating stage objects on React re-render                                                                                                                                                                                                                                                                                                                                                          |
