...
| `src/renderer/components/tokens/TokenForm.tsx`                 | Reusable token form for create/edit (name required, required grid-type select with default `square`, optional image URL, visible checkbox) plus token image upload payload wiring (`image_upload`) from dropzone/file picker, edit-mode current image preview, explicit clear-image submit signal (`clear_image`), and footprint painter modal integration for image uploads (opens `FootprintPainterModal` on file select, persists footprint/framing data to token config JSON)                                                                                                                                                                                                                                                                                 |
