...
- `src/database/db.ts -> initializeSchema()` currently creates `verses`, `worlds`, `levels`, `campaigns`, `sessions`, `abilities`, and `ability_children` via `CREATE TABLE IF NOT EXISTS` for migration-safe startup on existing user databases.
