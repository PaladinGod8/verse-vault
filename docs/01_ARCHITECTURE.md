...
- `src/shared/ipcChannels.ts` includes campaign channel constants added in Campaign Step 01 (2026-02-27): `CAMPAIGNS_GET_ALL_BY_WORLD`, `CAMPAIGNS_GET_BY_ID`, `CAMPAIGNS_ADD`, `CAMPAIGNS_UPDATE`, and `CAMPAIGNS_DELETE`; Campaign Step 07 wires these handlers in `main`, while preload bridge methods are still not wired.
- `src/shared/ipcChannels.ts` includes session channel constants added in Session Step 02 (2026-02-27): `SESSIONS_GET_ALL_BY_CAMPAIGN`, `SESSIONS_GET_BY_ID`, `SESSIONS_ADD`, `SESSIONS_UPDATE`, and `SESSIONS_DELETE`; Session Step 08 wires these handlers in `main`, while preload bridge methods are still not wired.
- `src/shared/ipcChannels.ts` includes scene channel constants added in Scene Step 03 (2026-02-27): `SCENES_GET_ALL_BY_SESSION`, `SCENES_GET_BY_ID`, `SCENES_ADD`, `SCENES_UPDATE`, and `SCENES_DELETE`; Scene Step 09 wires these handlers in `main`, while preload bridge methods are still not wired.
