...
4. **Channel names are constants.** All IPC channel strings live in `src/shared/ipcChannels.ts`. No magic strings in `main.ts` or `preload.ts`. Ability Step 01 (2026-02-27) adds abilities channel constants only; handler and preload wiring remains intentionally deferred in Step 02.
...
- `src/shared/ipcChannels.ts` now includes abilities channel constants for Step 01 (`ABILITIES_GET_ALL_BY_WORLD`, `ABILITIES_GET_BY_ID`, `ABILITIES_ADD`, `ABILITIES_UPDATE`, `ABILITIES_DELETE`, `ABILITIES_ADD_CHILD`, `ABILITIES_REMOVE_CHILD`, `ABILITIES_GET_CHILDREN`); these are contract-only in this step and not yet wired in `main` or `preload`.
