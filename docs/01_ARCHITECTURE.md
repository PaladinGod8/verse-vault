...
- `src/database/db.ts -> initializeSchema()` currently creates `verses`, `worlds`, `levels`, `campaigns`, `sessions`, `scenes`, `abilities`, and `ability_children` via `CREATE TABLE IF NOT EXISTS` for migration-safe startup on existing user databases.
...
- `sessions` schema baseline (Step 05, 2026-02-27): `id`, `campaign_id` (FK -> campaigns, `ON DELETE CASCADE`), `name`, `notes`, `sort_order` (default `0`), `created_at`, `updated_at`.
...
- `src/shared/ipcChannels.ts` includes session channel constants added in Session Step 02 (2026-02-27): `SESSIONS_GET_ALL_BY_CAMPAIGN`, `SESSIONS_GET_BY_ID`, `SESSIONS_ADD`, `SESSIONS_UPDATE`, and `SESSIONS_DELETE`; Session Step 08 wires these handlers in `main`; Step 10 (2026-02-28) wires preload bridges via `window.db.sessions`.
- `src/shared/ipcChannels.ts` includes scene channel constants added in Scene Step 03 (2026-02-27): `SCENES_GET_ALL_BY_SESSION`, `SCENES_GET_BY_ID`, `SCENES_ADD`, `SCENES_UPDATE`, and `SCENES_DELETE`; Scene Step 09 wires these handlers in `main`; Step 10 (2026-02-28) wires preload bridges via `window.db.scenes`.
