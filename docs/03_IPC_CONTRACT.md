...
Abilities Step 04 (2026-02-27) wires core mutation handlers in main for `ABILITIES_ADD`, `ABILITIES_UPDATE`, and `ABILITIES_DELETE` in addition to the Step 03 read handlers (`ABILITIES_GET_ALL_BY_WORLD`, `ABILITIES_GET_BY_ID`, `ABILITIES_GET_CHILDREN`). Preload bridge methods and parent-child mutation handlers remain intentionally unwired in this step.
...
- Ability parent-child mutation handlers (`ABILITIES_ADD_CHILD`, `ABILITIES_REMOVE_CHILD`) remain intentionally unwired.
