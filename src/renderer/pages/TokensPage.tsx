...
export default function TokensPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeletingToken((current) => current?.id === token.id ? null : current);
      setPendingDeleteToken((current) => current?.id === token.id ? null : current);
    }
  };
  ...
  const handleConfirmMove = async (token: Token, targetCampaignId?: number) => {
    ...
    try {
      if (moveDialogMode === 'toWorld') {
        ...
        setTokens((prev) => prev.map((t) => (t.id === updated.id ? updated : t)));
        const campaignName = campaigns.find((c) => c.id === targetCampaignId)?.name || 'Unknown';
                              ...
                              ? (
                                <>
                                  ...
                                  <button
                                    ...
                                    className='text-sm font-medium text-indigo-600 transition hover:text-indigo-800 disabled:cursor-not-allowed disabled:opacity-60'
                                    disabled={deletingToken?.id === token.id || isSaving}
                                  >
                                    Copy to Campaign
                                  </button>
                                </>
