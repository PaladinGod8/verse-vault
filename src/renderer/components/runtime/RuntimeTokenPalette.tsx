...
function tokenSourceKey(campaignId: number, sourceTokenId: number): string {
  return `${campaignId}:${sourceTokenId}`;
}
...
export default function RuntimeTokenPalette({
  ...
  const placedTokenSourceKeys = useMemo(() => {
    ...
    for (const placedToken of placedTokens) {
      if (
        placedToken.campaignId === null ||
        placedToken.sourceTokenId === null ||
        placedToken.sourceMissing
      ) {
        continue;
      }
      sourceKeys.add(
    ...
    <section className="space-y-4 border-b border-slate-800 px-6 py-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="space-y-3 rounded-lg border border-slate-800 bg-slate-950/60 p-3">
          ...
          {isLoadingTokens ? (
            <p className="text-xs text-slate-300">Loading campaign tokens...</p>
          ) : null}
        ...
        <div className="space-y-2 rounded-lg border border-slate-800 bg-slate-950/60 p-3">
          ...
          {placedTokens.length === 0 ? (
            <p className="text-xs text-slate-400">
              Add tokens from a campaign to place them in runtime.
            </p>
          ) : null}
