...
export default function StatBlockForm({
  ...
  const [config, setConfig] = useState(() => normalizeConfig(initialData?.config));
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const hasResources = Object.keys(filteredStatistics?.statistics?.resources ?? {}).length > 0;
      const hasPassiveScores = Object.keys(filteredStatistics?.statistics?.passiveScores ?? {})
        .length > 0;
      const finalConfig = filteredStatistics && (hasResources || hasPassiveScores)
    ...
    <form className='space-y-4' onSubmit={handleSubmit}>
      ...
      <div className='space-y-1'>
        ...
        <textarea
          ...
          className={`min-h-24 w-full rounded-lg border px-3 py-2 font-mono text-sm text-slate-900 transition outline-none focus:ring-2 focus:ring-slate-200 ${
            configError
