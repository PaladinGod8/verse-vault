...
export type BackgroundImageUploadPayload = {
  fileName: string;
  mimeType: string;
  bytes: Uint8Array;
};
...
export type BackgroundFormValues = {
  ...
  image_upload?: BackgroundImageUploadPayload;
  clear_image?: boolean;
};

type BackgroundFormProps = {
  initialValues?: BackgroundFormValues;
  onSave: (data: BackgroundFormValues) => Promise<void> | void;
  onClose: () => void;
  isSaving: boolean;
};
...
function validateBackgroundImageFile(file: File): string | null {
  ...
  if (!BACKGROUND_IMAGE_ALLOWED_MIME_TYPES.has(mimeType)) {
...
async function readBackgroundImageUpload(
  selectedImageFile: File | null,
): Promise<{
  imageUpload?: BackgroundImageUploadPayload;
  error?: string;
}> {
  ...
  const validationError = validateBackgroundImageFile(selectedImageFile);
  if (validationError) {
...
type BackgroundCurrentImageSectionProps = {
...
function BackgroundCurrentImageSection({
  initialImageSrc,
  previewUrl,
  clearImage,
  isSaving,
  onClear,
}: BackgroundCurrentImageSectionProps) {
  return (
...
export default function BackgroundForm({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const { imageUpload, error } = await readBackgroundImageUpload(selectedImageFile);
    if (error) {
