...
export default function BackgroundCard({
  ...
  const imageSrc = background.image_src?.trim() ?? '';
  ...
  const handleOpen = () => {
    navigate(`/world/${background.world_id}/backgrounds/${background.id}`);
  };
  ...
  return (
    <article
      role='button'
      tabIndex={0}
      aria-label={`Open ${background.name}`}
    ...
    >
      <div
        ...
        data-testid='background-card-media-frame'
      >
        {showImage
          ? (
            <img
