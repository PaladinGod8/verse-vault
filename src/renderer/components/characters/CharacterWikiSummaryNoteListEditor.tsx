type NoteListItem = { text: string; note?: string | null; };

type CharacterWikiSummaryNoteListEditorProps = {
  legend: string;
  items: NoteListItem[];
  onChange: (items: NoteListItem[]) => void;
  disabled?: boolean;
};

export default function CharacterWikiSummaryNoteListEditor({
  legend,
  items,
  onChange,
  disabled = false,
}: CharacterWikiSummaryNoteListEditorProps) {
  const updateItem = (index: number, patch: Partial<NoteListItem>) => {
    const next = [...items];
  ...
  return (
    <fieldset className='space-y-2'>
      ...
      <button
        ...
        onClick={() => onChange([...items, { text: '', note: '' }])}
        disabled={disabled}
      >
        Add {legend}
      </button>
