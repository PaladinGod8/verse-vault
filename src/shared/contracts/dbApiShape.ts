...
import type { DbApiRelationships } from './dbApiRelationships';
import type {
  Ability,
  AbilityChild,
  Act,
...
export interface DbApi extends DbApiRelationships {
  ...
  backgrounds: {
    getAllByWorld(worldId: number): Promise<Background[]>;
    getById(id: number): Promise<Background | null>;
    add(data: BackgroundUpsertPayload & { world_id: number; name: string; }): Promise<Background>;
    update(id: number, data: BackgroundUpsertPayload): Promise<Background>;
    delete(id: number): Promise<{ id: number; }>;
    importImage(payload: TokenImageImportPayload): Promise<TokenImageImportResult>;
