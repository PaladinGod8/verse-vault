...
import ItemImageDropzone from '../../../src/renderer/components/items/ItemImageDropzone';
...
describe('ItemImageDropzone', () => {
  it('renders helper copy and accepts file selection from input', () => {
    ...
    expect(screen.getByText('Item Image Upload')).toBeInTheDocument();
  ...
  it('shows selected file metadata and allows clearing', () => {
    ...
    const file = makeImageFile('sunblade.webp', '', new Uint8Array(2 * 1024 * 1024));
    render(
      <ItemImageDropzone
        selectedFile={file}
        onFileSelect={vi.fn()}
    ...
    expect(screen.getByText('sunblade.webp')).toBeInTheDocument();
