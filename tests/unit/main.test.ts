...
describe('main process', () => {
  ...
  it('handles statblock linkage and assignment IPC success and validation errors', async () => {
    ...
    const statblocksByIdGetMock = vi.fn((id: number) => {
      ...
      if (id === 92) {
        return { ...statblockRow, id, world_id: 2 };
      }
    ...
    const tokensByIdGetMock = vi.fn((id: number) => {
      ...
      if (id === 72) {
        return { ...linkedTokenRow, id, world_id: 2 };
      }
      return { ...linkedTokenRow, id };
    });
    const abilitiesByIdGetMock = vi.fn((id: number) => {
      ...
      if (id === 22) {
        return { ...linkedAbilityRow, id, world_id: 2 };
      }
      return { ...linkedAbilityRow, id };
    });
    ...
    const linkedTokensAllMock = vi.fn(() => [{ ...linkedTokenRow, id: 81 }]);
    ...
    const attachAbilityRunMock = vi.fn(
      (statblockId: number, abilityId: number) => {
        if (abilityId === 23) {
          throw new Error(
    ...
    const listAbilitiesAllMock = vi.fn(() => [{ ...linkedAbilityRow, id: 31 }]);

    prepareMock.mockImplementation((sql: string) => {
      if (sql === 'SELECT * FROM statblocks WHERE id = ?') {
        return { get: statblocksByIdGetMock };
      }
      if (sql === 'SELECT * FROM tokens WHERE id = ?') {
        return { get: tokensByIdGetMock };
      }
      if (sql === 'SELECT * FROM abilities WHERE id = ?') {
        return { get: abilitiesByIdGetMock };
      }
      if (
        sql
    ...
    const linkResult = registeredIpcHandlers[IPC.STATBLOCKS_LINK_TOKEN]({}, {
      statblock_id: 91,
      token_id: 71,
    });
    ...
    expect(() =>
      registeredIpcHandlers[IPC.STATBLOCKS_LINK_TOKEN]({}, {
        statblock_id: 404,
        token_id: 71,
      })
    ...
    const unlinkResult = registeredIpcHandlers[IPC.STATBLOCKS_UNLINK_TOKEN]({}, {
      statblock_id: 91,
      token_id: 71,
    });
    ...
    const linkedTokensResult = registeredIpcHandlers[
      IPC.STATBLOCKS_GET_LINKED_TOKENS
    ]({}, 91);
    expect(linkedTokensAllMock).toHaveBeenCalledWith(91);
    expect(linkedTokensResult).toEqual([{ ...linkedTokenRow, id: 81 }]);
    const linkedTokensSql = prepareMock.mock.calls.find(
      ([sql]) =>
        typeof sql === 'string'
        && sql.includes('FROM statblock_token_links AS link')
        && sql.includes('ORDER BY token.updated_at DESC, token.id DESC'),
    )?.[0];
    expect(linkedTokensSql).toContain('ORDER BY token.updated_at DESC, token.id DESC');

    const linkedStatblockResult = registeredIpcHandlers[
      IPC.STATBLOCKS_GET_LINKED_STATBLOCK
    ]({}, 71);
    expect(linkedStatblockGetMock).toHaveBeenCalledWith(71);
    expect(linkedStatblockResult).toMatchObject({ id: 101 });

    const missingLinkedStatblockResult = registeredIpcHandlers[
      IPC.STATBLOCKS_GET_LINKED_STATBLOCK
    ]({}, 999);
    expect(missingLinkedStatblockResult).toBeNull();

    const attachResult = registeredIpcHandlers[IPC.STATBLOCKS_ATTACH_ABILITY]({}, {
      statblock_id: 91,
      ability_id: 21,
    });
    ...
    expect(() =>
      registeredIpcHandlers[IPC.STATBLOCKS_ATTACH_ABILITY]({}, {
        statblock_id: 404,
        ability_id: 21,
      })
    ).toThrowError('StatBlock not found');
    ...
    const detachResult = registeredIpcHandlers[IPC.STATBLOCKS_DETACH_ABILITY]({}, {
      statblock_id: 91,
      ability_id: 21,
    });
    ...
    const listedAbilitiesResult = registeredIpcHandlers[
      IPC.STATBLOCKS_LIST_ABILITIES
    ]({}, 91);
    expect(listAbilitiesAllMock).toHaveBeenCalledWith(91);
    expect(listedAbilitiesResult).toEqual([{ ...linkedAbilityRow, id: 31 }]);
    const listAbilitiesSql = prepareMock.mock.calls.find(
      ([sql]) =>
        typeof sql === 'string'
        && sql.includes('FROM statblock_ability_assignments AS assignment')
        && sql.includes('ORDER BY ability.updated_at DESC, ability.id DESC'),
    )?.[0];
    expect(listAbilitiesSql).toContain(
