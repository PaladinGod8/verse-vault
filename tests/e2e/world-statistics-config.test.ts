...
test.describe('@ux @db World Statistics Configuration', () => {
  async function withStatisticsPage(run: (page: Page) => Promise<void>) {
    const context = await launchElectronApp();

    try {
      await prepareWindow(context.page);
  ...
  test('shows default resource and passive score definitions for a new world', async () => {
    await withStatisticsPage(async (page) => {
